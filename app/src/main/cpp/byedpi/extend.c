...
static int protect(int conn_fd, const char *path)
{
    ...
    // Abstract namespace (leading NUL) so an Android LocalServerSocket can bind
    // to it by name. FRKN passes the abstract socket name via --protect-path.
    sa.sun_path[0] = '\0';
    ...
    int err = connect(fd, (struct sockaddr *)&sa, addrlen);
    if (err) {
        uniperror("connect");
        close(fd);
        return -1;
